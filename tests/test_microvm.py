...
import string
import pytest
from conftest import KERNEL_FILE, BASE_ROOTFS, check_kvm_available
...
def test_create_with_invalid_rootfs_path():
    ...
    with pytest.raises(
        VMMError, match=r"Failed to create VMM .*: Base rootfs not found:"
    ):
        vm.create()
...
def test_create_with_invalid_kernel_file():
    ...
    with pytest.raises(
        VMMError, match=r"Failed to create VMM .*: Kernel file not found:"
    ):
        vm.create()
...
def test_create_with_missing_base_rootfs_for_docker():
    ...
    with pytest.raises(
        ValueError, match=r"base_rootfs is required when image is provided"
    ):
...
def test_vmm_create_multiple_vms():
    ...
    for i in range(num_vms):
        ...
        vm = MicroVM(
            ip_addr=unique_ip, kernel_file=KERNEL_FILE, base_rootfs=BASE_ROOTFS
        )
    ...
    assert len(created_vms) == num_vms, (
        f"Expected {num_vms} VMs to be created, but only {len(created_vms)} were created"
    )
...
def test_vmm_list():
    ...
    assert vms[0]["ip_addr"] == "172.16.0.2", (
        "VM IP address should match the created IP address"
    )
...
def test_port_forwarding():
    ...
    # Add port forwarding again to complete the test
    result = vm.port_forward(host_port=host_port, dest_port=dest_port)
    assert f"Port forwarding added successfully for VMM {id}" in result
...
def test_vmm_delete():
    ...
    vm = MicroVM(
        ip_addr="172.16.0.32", kernel_file=KERNEL_FILE, base_rootfs=BASE_ROOTFS
    )
    vm.create()
    ...
    assert list_result[0]["id"] == id, (
        f"Expected VM ID {id}, but got {list_result[0]['id']}"
    )

    # Check if config.json exists before deletion
    config_path = f"/var/lib/firecracker/{id}/config.json"
    assert os.path.exists(config_path), (
        f"config.json not found at {config_path}, cannot proceed with deletion"
    )

    # Delete the VM using the name
    delete_result = vm.delete()
    assert f"VMM {id} is deleted" in delete_result, (
        f"Unexpected delete result: {delete_result}"
    )
